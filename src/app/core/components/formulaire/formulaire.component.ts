  ...
  if (this.reservationForm.valid && !this.isSubmitting) {
    ...
    this.reservationService.createReservation(reservationData).subscribe({
      ...
      error: (err) => {
        ...
        if (typeof message === 'string' && message.includes('déjà réservés')) {
...
private extractErrorMessage(err: any): string {
  ...
  // Si le backend retourne un message direct (chaîne de caractères)
  if (typeof err.error === 'string') {
    return err.error;
  }

  // Si c’est un objet structuré avec un champ "message"
  if (err.error?.message) {
    return err.error.message;
  }

  // Erreur de type réseau (ex. pas de connexion)
  if (err.error instanceof ErrorEvent) {
    return 'Problème de connexion internet';
  }

  // Erreurs de type 4xx ou 5xx
  if (err.status >= 500) {
  ...
  // Cas par défaut (ex. validation, date réservée, etc.)
  return 'La date est déjà réservée ou invalide.';
}
}
